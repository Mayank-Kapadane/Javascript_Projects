...
async function fetchRandomNews() {
    try {
        ...
        */
        ...
        // For fetching the data visit the homepage of website(newsapi) and there are some examples. of data in json format.
        ...
        // the parameter pageSize=10 means we're showing 10 blocks.
        // different type of parameter you can search on `newsapi` website.
...
searchButton.addEventListener("click", async (e) => {
    ...
    if (query === "") {
        ...
        try {
            // again fetch data as per the query
            const articles = await fetchNewsQuery(query);
            displayBlogs(articles);
        } catch (error) {
            console.log("Error fetching news from input query", error);
        }
    }
});

async function fetchNewsQuery(query) {
    console.log("calling fetch news query ...");
    
    try {
        const apiUrl =
            `https://newsapi.org/v2/everything?q=${query}&pageSize=20&apiKey=` +
            API_KEY;
        ...
        console.log("Error ferching news by query ", error);
        return [];
    }
}
...
function displayBlogs(articles) {
    ...
    articles.forEach((article) => {
        ...
        */
        ...
        // console.log(!Boolean(article.urlToImage))
        // // console.log(article.urlToImage)
        // if (!Boolean(article.urlToImage)){                         // if the image is "null" 
        ...
        img.onerror = function () {
            ...
            // If the placeholder image itself failed to load (rare, but possible), onerror would run again and again.
        ...
        */
        ...
        // If someone click on the card then we want to show the News in new window
...
// Moon bracket function or IIFE(Immediate invoke function)
(async () => {
    /* 
    console.log("hi");
    ********************
